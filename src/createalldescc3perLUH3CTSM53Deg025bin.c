...
char namelistluh3descallc3perallcftname[1024];
char namelistluh3descrawcftoutput[1024];
char namelistluh3descrawcftoutputc3pername[1024];
...
float inCTSMPCTC3PERCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
float outDESCPCTC3PERCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descallc3perallcftname);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutput);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutputc3pername);
...
int initializeGrids() {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
      for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
          ...
          inCTSMPCTC3PERCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          ...
          outDESCPCTC3PERCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
...
int readclm5cropGrids(char *databasestr,char *seriesname,char *referenceyearname,char *croptype) {
  ...
  for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
      if (strcmp(CFTluhtype[cftid],croptype) == 0) {
          for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
              for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
	          inCTSMPCTC3PERCROPGrid[clm5lin * MAXCLMPIX + clm5pix] += inCTSMPCTCFTGrid[cftid][clm5lin * MAXCLMPIX + clm5pix];
              }
	  }
      }
  }
      ...
      if (searchlin >= 0 && searchlin < MAXCLMLIN) {
		  ...
		  pixdistance = ((float) (centerpix - searchpix)) * CLMPIXSIZE;
		      ...
		      if (landfracvalue > 0.0) {
                          cropvalue = inCTSMPCTC3PERCROPGrid[searchlin * MAXCLMPIX + searchpix];
...
int genLUHDescriptorCropGrids(char *croptype) {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      currentcroppct = inCTSMPCTC3PERCROPGrid[clm5lin * MAXCLMPIX + clm5pix];
...
int main(long narg, char **argv) {
  ...
  if(narg != 9){
        printf("Usage createalldescc3perCTSM53Deg025bin luh3descrawfile luh3descrawdir timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3desccropparams luh3descc3perpftnamelistfile\n");
        return 0;
  }
  ...
  for (currentyear = clm5modisrawinputstartyear; currentyear <= clm5modisrawinputendyear; currentyear++) {
      ...
      clearGrids();
      readclm5cropGrids(clm5modisrawinputdb,clm5modisrawinputname,clm5modisrawinputreferenceyearstr,"C3PER");
      genLUHDescriptorCropGrids("C3PER");
      regroupDescriptorCropGrids("C3PER");
      writedescgrids(luh3descrawcftoutputdirname,namelistluh3descrawcftoutputc3pername,luh3descrawcftoutputreferenceyearstr);
  }
