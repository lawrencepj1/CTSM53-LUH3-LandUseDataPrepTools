...
char namelistluh3descallprimnallpftname[1024];
char namelistluh3descrawpftoutput[1024];
char namelistluh3descrawpftoutputprimnname[1024];
...
float inMERGEDPRIMNFRACTREEGrid[MAXCLMPIX * MAXCLMLIN];
float inMERGEDPRIMNFRACHERBGrid[MAXCLMPIX * MAXCLMLIN];
float inMERGEDPRIMNFRACBAREGrid[MAXCLMPIX * MAXCLMLIN];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descallprimnallpftname);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawpftoutput);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawpftoutputprimnname);
...
int readmworkergedPRIMNGrids(char *databasestr,char *seriesname,char *referenceyearname) {
  ...
  sprintf(globalbinfilename,"%s/%s/%s.PRIMNFRACTREE.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMNFRACTREEGrid,sizeof(inMERGEDPRIMNFRACTREEGrid),1,globalbinfile);  
  fclose(globalbinfile);

  sprintf(globalbinfilename,"%s/%s/%s.PRIMNFRACHERB.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMNFRACHERBGrid,sizeof(inMERGEDPRIMNFRACHERBGrid),1,globalbinfile);  
  fclose(globalbinfile);

  sprintf(globalbinfilename,"%s/%s/%s.PRIMNFRACBARE.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMNFRACBAREGrid,sizeof(inMERGEDPRIMNFRACBAREGrid),1,globalbinfile);  
  fclose(globalbinfile);
...
int genMergedDescriptorVegGrids() {
  ...
  float potvegmaxtreepct, potvegscaledtreepct, mergedprimnfractree;
  float potvegmaxherbpct, potvegscaledherbpct, mergedprimnfracherb;
  float potvegmaxbarepct, potvegscaledbarepct, mergedprimnfracbare;
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      mergedprimnfractree = inMERGEDPRIMNFRACTREEGrid[clm5lin * MAXCLMPIX + clm5pix];
	      nonforesttreepct = mergedprimnfractree * potvegmaxtreepct + (1.0 - mergedprimnfractree) * potvegscaledtreepct;
	      ...
	      mergedprimnfracherb = inMERGEDPRIMNFRACHERBGrid[clm5lin * MAXCLMPIX + clm5pix];
	      nonforestherbpct = mergedprimnfracherb * potvegmaxherbpct + (1.0 - mergedprimnfracherb) * potvegscaledherbpct;
	      ...
	      mergedprimnfracbare = inMERGEDPRIMNFRACBAREGrid[clm5lin * MAXCLMPIX + clm5pix];
	      nonforestbarepct = mergedprimnfracbare * potvegmaxbarepct + (1.0 - mergedprimnfracbare) * potvegscaledbarepct;
...
int main(long narg, char **argv) {
  ...
  if(narg != 8){
        printf("Usage createalldescprimnCTSM53Deg025bin luh3descrawfile luh3descrawdir timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3descprimnpftnamelistfile\n");
        return 0;
  }
  ...
  for (currentyear = workrawmergeinputstartyear; currentyear <= workrawmergeinputendyear; currentyear++) {
      ...
      clearGrids();
      readmworkergedPRIMNGrids(workrawmergeinputdirname,namelistworkrawmergeinputname,workrawmergeinputreferenceyearstr);
      genMergedDescriptorVegGrids();
      balanceMergedDescriptorVegGrids();
      writedescmergedgrids(luh3descrawpftoutputdirname,namelistluh3descrawpftoutputprimnname,luh3descrawpftoutputreferenceyearstr);
  }
