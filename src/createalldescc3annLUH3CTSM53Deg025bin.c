...
char namelistluh3descallc3annallcftname[1024];
char namelistluh3descrawcftoutput[1024];
char namelistluh3descrawcftoutputc3annname[1024];
...
float inCTSMPCTC3ANNCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
float outDESCPCTC3ANNCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descallc3annallcftname);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutput);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutputc3annname);
...
int initializeGrids() {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
      for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
          ...
          inCTSMPCTC3ANNCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          ...
          outDESCPCTC3ANNCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
...
int readclm5cropGrids(char *databasestr,char *seriesname,char *referenceyearname,char *croptype) {
  ...
  for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
      if (strcmp(CFTluhtype[cftid],croptype) == 0) {
          for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
              for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
	          inCTSMPCTC3ANNCROPGrid[clm5lin * MAXCLMPIX + clm5pix] += inCTSMPCTCFTGrid[cftid][clm5lin * MAXCLMPIX + clm5pix];
              }
	  }
      }
  }
      ...
      if (searchlin >= 0 && searchlin < MAXCLMLIN) {
		  ...
		  pixdistance = ((float) (centerpix - searchpix)) * CLMPIXSIZE;
		      ...
		      if (landfracvalue > 0.0) {
                          cropvalue = inCTSMPCTC3ANNCROPGrid[searchlin * MAXCLMPIX + searchpix];
...
int genLUHDescriptorCropGrids(char *croptype) {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      currentcroppct = inCTSMPCTC3ANNCROPGrid[clm5lin * MAXCLMPIX + clm5pix];
...
int regroupDescriptorCropGrids(char *croptype) {
  ...
  float luhc3croppct, luhcftpct, luhcftlevel, luhcftpctsum, foddergrasspad;
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      extraplevel = minextraplevel;
              ...
              if (extraplevel == 0.0) {
	          luhc3croppct = outDESCPCTCROPGrid[clm5lin * MAXCLMPIX + clm5pix];
                  totalcftpct = 0.0;
	          cftcount = 0;
	          for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
                      cftpct = outDESCPCTCFTGrid[cftid][clm5lin * MAXCLMPIX + clm5pix];
...
int main(long narg, char **argv) {
  ...
  if(narg != 9){
        printf("Usage createalldescc3annCTSM53Deg025bin luh3descrawfile luh3descrawdir timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3desccropparams luh3descc3annpftnamelistfile\n");
        return 0;
  }
  ...
  for (currentyear = clm5modisrawinputstartyear; currentyear <= clm5modisrawinputendyear; currentyear++) {
      ...
      clearGrids();
      readclm5cropGrids(clm5modisrawinputdb,clm5modisrawinputname,clm5modisrawinputreferenceyearstr,"C3ANN");
      genLUHDescriptorCropGrids("C3ANN");
      regroupDescriptorCropGrids("C3ANN");
      writedescgrids(luh3descrawcftoutputdirname,namelistluh3descrawcftoutputc3annname,luh3descrawcftoutputreferenceyearstr);
  }
