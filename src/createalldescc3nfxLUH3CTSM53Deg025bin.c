...
char namelistluh3descallc3nfxallcftname[1024];
char namelistluh3descrawcftoutput[1024];
char namelistluh3descrawcftoutputc3nfxname[1024];
...
float inCTSMPCTC3NFXCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
float outDESCPCTC3NFXCROPGrid[MAXCLMPIX * MAXCLMLIN];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descallc3nfxallcftname);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutput);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawcftoutputc3nfxname);
...
int initializeGrids() {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
      for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
          ...
          inCTSMPCTC3NFXCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          ...
          outDESCPCTC3NFXCROPGrid[clm5lin * MAXCLMPIX + clm5pix] = 0.0;
          for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
...
int readclm5cropGrids(char *databasestr,char *seriesname,char *referenceyearname,char *croptype) {
  ...
  for (cftid = 0; cftid < MAXCFTLUH3; cftid++) {
      if (strcmp(CFTluhtype[cftid],croptype) == 0) {
          for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
              for (clm5pix = 0; clm5pix < MAXCLMPIX; clm5pix++) {
	          inCTSMPCTC3NFXCROPGrid[clm5lin * MAXCLMPIX + clm5pix] += inCTSMPCTCFTGrid[cftid][clm5lin * MAXCLMPIX + clm5pix];
              }
	  }
      }
  }
      ...
      if (searchlin >= 0 && searchlin < MAXCLMLIN) {
		  ...
		  pixdistance = ((float) (centerpix - searchpix)) * CLMPIXSIZE;
		      ...
		      if (landfracvalue > 0.0) {
                          cropvalue = inCTSMPCTC3NFXCROPGrid[searchlin * MAXCLMPIX + searchpix];
...
int genLUHDescriptorCropGrids(char *croptype) {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      currentcroppct = inCTSMPCTC3NFXCROPGrid[clm5lin * MAXCLMPIX + clm5pix];
...
int main(long narg, char **argv) {
  ...
  if(narg != 9){
        printf("Usage createalldescc3nfxCTSM53Deg025bin luh3descrawfile luh3descrawdir timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3desccropparams luh3descc3nfxpftnamelistfile\n");
        return 0;
  }
  ...
  for (currentyear = clm5modisrawinputstartyear; currentyear <= clm5modisrawinputendyear; currentyear++) {
      ...
      clearGrids();
      readclm5cropGrids(clm5modisrawinputdb,clm5modisrawinputname,clm5modisrawinputreferenceyearstr,"C3NFX");
      genLUHDescriptorCropGrids("C3NFX");
      regroupDescriptorCropGrids("C3NFX");
      writedescgrids(luh3descrawcftoutputdirname,namelistluh3descrawcftoutputc3nfxname,luh3descrawcftoutputreferenceyearstr);
  }
