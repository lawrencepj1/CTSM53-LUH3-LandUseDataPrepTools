...
char namelistluh3descallprimfallpftname[1024];
char namelistluh3descrawpftoutput[1024];
char namelistluh3descrawpftoutputprimfname[1024];
...
float inMERGEDPRIMFFRACTREEGrid[MAXCLMPIX * MAXCLMLIN];
float inMERGEDPRIMFFRACHERBGrid[MAXCLMPIX * MAXCLMLIN];
float inMERGEDPRIMFFRACBAREGrid[MAXCLMPIX * MAXCLMLIN];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descallprimfallpftname);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawpftoutput);
  fscanf(namelistfile,"%s %s",fieldname,namelistluh3descrawpftoutputprimfname);
...
int readmworkergedPRIMFGrids(char *databasestr,char *seriesname,char *referenceyearname) {
  ...
  sprintf(globalbinfilename,"%s/%s/%s.PRIMFFRACTREE.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMFFRACTREEGrid,sizeof(inMERGEDPRIMFFRACTREEGrid),1,globalbinfile);  
  fclose(globalbinfile);

  sprintf(globalbinfilename,"%s/%s/%s.PRIMFFRACHERB.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMFFRACHERBGrid,sizeof(inMERGEDPRIMFFRACHERBGrid),1,globalbinfile);  
  fclose(globalbinfile);

  sprintf(globalbinfilename,"%s/%s/%s.PRIMFFRACBARE.%s.dat",databasestr,seriesname,seriesname,referenceyearname);
  printf("Reading: %s\n",globalbinfilename);
  globalbinfile = fopen(globalbinfilename,"r");
  fread(inMERGEDPRIMFFRACBAREGrid,sizeof(inMERGEDPRIMFFRACBAREGrid),1,globalbinfile);  
  fclose(globalbinfile);
...
int genMergedDescriptorVegGrids() {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      mergedprimffractree = inMERGEDPRIMFFRACTREEGrid[clm5lin * MAXCLMPIX + clm5pix];
	      foresttreepct = mergedprimffractree * potvegmaxtreepct + (1.0 - mergedprimffractree) * potvegscaledtreepct;
	      ...
	      mergedprimffracherb = inMERGEDPRIMFFRACHERBGrid[clm5lin * MAXCLMPIX + clm5pix];
	      forestherbpct = mergedprimffracherb * potvegmaxherbpct + (1.0 - mergedprimffracherb) * potvegscaledherbpct;
	      ...
	      mergedprimffracbare = inMERGEDPRIMFFRACBAREGrid[clm5lin * MAXCLMPIX + clm5pix];
	      forestbarepct = mergedprimffracbare * potvegmaxbarepct + (1.0 - mergedprimffracbare) * potvegscaledbarepct;
...
int main(long narg, char **argv) {
  ...
  if(narg != 8){
        printf("Usage createalldescprimfCTSM53Deg025bin luh3descrawfile luh3descrawdir timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3descprimfpftnamelistfile\n");
        return 0;
  }
  ...
  for (currentyear = workrawmergeinputstartyear; currentyear <= workrawmergeinputendyear; currentyear++) {
      ...
      clearGrids();
      readmworkergedPRIMFGrids(workrawmergeinputdirname,namelistworkrawmergeinputname,workrawmergeinputreferenceyearstr);
      genMergedDescriptorVegGrids();
      balanceMergedDescriptorVegGrids();
      writedescmergedgrids(luh3descrawpftoutputdirname,namelistluh3descrawpftoutputprimfname,luh3descrawpftoutputreferenceyearstr);
  }
