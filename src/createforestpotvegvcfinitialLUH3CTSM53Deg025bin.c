...
char namelistforesttreeclimatename[1024];
char namelistforestherbclimatename[1024];
char namelistforestbareclimatename[1024];
...
char foresttreeclimatename[1024];
char foresttreeclimatedb[1024];
int foresttreeclimatestartyear;
int foresttreeclimateendyear;

char forestherbclimatename[1024];
char forestherbclimatedb[1024];
int forestherbclimatestartyear;
int forestherbclimateendyear;

char forestbareclimatename[1024];
char forestbareclimatedb[1024];
int forestbareclimatestartyear;
int forestbareclimateendyear;

float forestpotentialtree[PRECIPCLASSES][TEMPCLASSES];
float forestpotentialherb[PRECIPCLASSES][TEMPCLASSES];
float forestpotentialbare[PRECIPCLASSES][TEMPCLASSES];
...
int readnamelistfile(char *namelist) {
  ...
  fscanf(namelistfile,"%s %s",fieldname,namelistforesttreeclimatename);
  fscanf(namelistfile,"%s %s",fieldname,namelistforestherbclimatename);
  fscanf(namelistfile,"%s %s",fieldname,namelistforestbareclimatename);
...
int readforesttreeclimatelutfile(char *filenamestr, char *dirnamestr, char *namestr) {

  FILE *foresttreeclimatevaluefile;
  int foundforesttreeclimate, itemsfound;
  char fullfilenamestr[1024], inforesttreeclimatename[1024], inforesttreeclimatedb[1024], inforesttreeclimatefile[1024];
  char inforesttreeclimatestartyear[1024], inforesttreeclimateendyear[1024];
  
  sprintf(fullfilenamestr,"%s",filenamestr);
  printf("Reading %s\n",fullfilenamestr);
  foresttreeclimatevaluefile = fopen(fullfilenamestr,"r");
  
  foundforesttreeclimate = 0;

  while (foundforesttreeclimate == 0) {
      itemsfound = fscanf(foresttreeclimatevaluefile,"%s %s %s %s %s",inforesttreeclimatename, inforesttreeclimatedb, inforesttreeclimatefile, inforesttreeclimatestartyear, inforesttreeclimateendyear);
      if (itemsfound != 5) {
          printf("Error: foresttreeclimate %s Not Found\n",namestr);
	  exit(0);
      }
      if (strcmp(namestr,inforesttreeclimatename) == 0) {
          printf("Processing foresttreeclimate: %s\n",inforesttreeclimatename);
	  sprintf(foresttreeclimatename,"%s",inforesttreeclimatename);
          if (strcmp(inforesttreeclimatedb,"<luh3modiscrurawdir>") == 0) {
              sprintf(foresttreeclimatedb,"%s%s",dirnamestr,inforesttreeclimatefile);
	  }
	  else {
              sprintf(foresttreeclimatedb,"%s%s",inforesttreeclimatedb,inforesttreeclimatefile);
	  }
          foresttreeclimatestartyear = atoi(inforesttreeclimatestartyear);
          foresttreeclimateendyear = atoi(inforesttreeclimateendyear);
	  foundforesttreeclimate = 1;
      }
  }  
...
int readforestherbclimatelutfile(char *filenamestr, char *dirnamestr, char *namestr) {

  FILE *forestherbclimatevaluefile;
  int foundforestherbclimate, itemsfound;
  char fullfilenamestr[1024], inforestherbclimatename[1024], inforestherbclimatedb[1024], inforestherbclimatefile[1024];
  char inforestherbclimatestartyear[1024], inforestherbclimateendyear[1024];
  
  sprintf(fullfilenamestr,"%s",filenamestr);
  printf("Reading %s\n",fullfilenamestr);
  forestherbclimatevaluefile = fopen(fullfilenamestr,"r");
  
  foundforestherbclimate = 0;

  while (foundforestherbclimate == 0) {
      itemsfound = fscanf(forestherbclimatevaluefile,"%s %s %s %s %s",inforestherbclimatename, inforestherbclimatedb, inforestherbclimatefile, inforestherbclimatestartyear, inforestherbclimateendyear);
      if (itemsfound != 5) {
          printf("Error: forestherbclimate %s Not Found\n",namestr);
	  exit(0);
      }
      if (strcmp(namestr,inforestherbclimatename) == 0) {
          printf("Processing forestherbclimate: %s\n",inforestherbclimatename);
	  sprintf(forestherbclimatename,"%s",inforestherbclimatename);
          if (strcmp(inforestherbclimatedb,"<luh3modiscrurawdir>") == 0) {
              sprintf(forestherbclimatedb,"%s%s",dirnamestr,inforestherbclimatefile);
	  }
	  else {
              sprintf(forestherbclimatedb,"%s%s",inforestherbclimatedb,inforestherbclimatefile);
	  }
          forestherbclimatestartyear = atoi(inforestherbclimatestartyear);
          forestherbclimateendyear = atoi(inforestherbclimateendyear);
	  foundforestherbclimate = 1;
      }
  }  
...
int readforestbareclimatelutfile(char *filenamestr, char *dirnamestr, char *namestr) {

  FILE *forestbareclimatevaluefile;
  int foundforestbareclimate, itemsfound;
  char fullfilenamestr[1024], inforestbareclimatename[1024], inforestbareclimatedb[1024], inforestbareclimatefile[1024];
  char inforestbareclimatestartyear[1024], inforestbareclimateendyear[1024];
  
  sprintf(fullfilenamestr,"%s",filenamestr);
  printf("Reading %s\n",fullfilenamestr);
  forestbareclimatevaluefile = fopen(fullfilenamestr,"r");
  
  foundforestbareclimate = 0;

  while (foundforestbareclimate == 0) {
      itemsfound = fscanf(forestbareclimatevaluefile,"%s %s %s %s %s",inforestbareclimatename, inforestbareclimatedb, inforestbareclimatefile, inforestbareclimatestartyear, inforestbareclimateendyear);
      if (itemsfound != 5) {
          printf("Error: forestbareclimate %s Not Found\n",namestr);
	  exit(0);
      }
      if (strcmp(namestr,inforestbareclimatename) == 0) {
          printf("Processing forestbareclimate: %s\n",inforestbareclimatename);
	  sprintf(forestbareclimatename,"%s",inforestbareclimatename);
          if (strcmp(inforestbareclimatedb,"<luh3modiscrurawdir>") == 0) {
              sprintf(forestbareclimatedb,"%s%s",dirnamestr,inforestbareclimatefile);
	  }
	  else {
              sprintf(forestbareclimatedb,"%s%s",inforestbareclimatedb,inforestbareclimatefile);
	  }
          forestbareclimatestartyear = atoi(inforestbareclimatestartyear);
          forestbareclimateendyear = atoi(inforestbareclimateendyear);
	  foundforestbareclimate = 1;
      }
  }  
...
int readpotentialveglut(char *filenamestr, int veglutid) {
  ...
  for (precipannindex = 0; precipannindex < PRECIPCLASSES; precipannindex++) {
	  ...
	  tempfloatvalue = atof(temptokenvalue);
          switch (veglutid) {
              case 1 : 
                  forestpotentialtree[precipannindex][tempaverageindex] = tempfloatvalue;
                  break;
              case 2 : 
                  forestpotentialherb[precipannindex][tempaverageindex] = tempfloatvalue;
                  break;
              case 3 : 
                  forestpotentialbare[precipannindex][tempaverageindex] = tempfloatvalue;
                  break;
          }
...
int printpotentialveglut(int veglutid) {
  ...
  for (precipannindex = 0; precipannindex < PRECIPCLASSES; precipannindex++) {
      ...
      for (tempaverageindex = 0; tempaverageindex < TEMPCLASSES; tempaverageindex++) {
          ...
          switch (veglutid) {
              case 1 : 
                  printf("%f %f - %f\n",precipannvalue,tempaveragevalue,forestpotentialtree[precipannindexcalc][tempaverageindexcalc]);
                  break;
              case 2 : 
                  printf("%f %f - %f\n",precipannvalue,tempaveragevalue,forestpotentialherb[precipannindexcalc][tempaverageindexcalc]);
                  break;
              case 3 : 
                  printf("%f %f - %f\n",precipannvalue,tempaveragevalue,forestpotentialbare[precipannindexcalc][tempaverageindexcalc]);
                  break;
          }
      }
  }
...
int genLUHPotentialVegGrids() {
  ...
  for (clm5lin = 0; clm5lin < MAXCLMLIN; clm5lin++) {
	  ...
	  if (landfrac > 0.0) {
	      ...
	      potvegtreepct = forestpotentialtree[precipannindex][tempaverageindex];
	      potvegherbpct = forestpotentialherb[precipannindex][tempaverageindex];
	      potvegbarepct = forestpotentialbare[precipannindex][tempaverageindex];
	      ...
	      if (currenttreepct > potvegtreepct) {
	          removepotveg = currenttreepct - potvegtreepct;
		  availpotveg = potvegherbpct;
	          if (removepotveg > availpotveg) {
		      potvegtreepct += potvegherbpct;
		      potvegherbpct = 0.0;
		  }
		  else {
		      potvegtreepct = currenttreepct;
		      potvegherbpct -= removepotveg;
		  }
              }
		      
	      if (potvegherbpct > currentherbpct) {
	          addpotveg = potvegherbpct - currentherbpct;
		  potvegherbpct -= addpotveg;
		  potvegtreepct += addpotveg;
              }
...
int main(long narg, char **argv) {
  ...
  if(narg != 8){
        printf("Usage createpotvegpftsclm552Deg025bin timeseriesrawfile timeseriesrawdir workrawfile workrawdir luh3modiscruclimatefile luh3modiscrurawdir generateforestpotveginitialvcfnamelist\n");
        return 0;
  }
  ...
  readforesttreeclimatelutfile(argv[5],argv[6],namelistforesttreeclimatename);
  readforestherbclimatelutfile(argv[5],argv[6],namelistforestherbclimatename);
  readforestbareclimatelutfile(argv[5],argv[6],namelistforestbareclimatename);
  ...
  readpotentialveglut(foresttreeclimatedb,1);
  readpotentialveglut(forestherbclimatedb,2);
  readpotentialveglut(forestbareclimatedb,3);
