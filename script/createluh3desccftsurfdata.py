...
if (arguments != 8):
    print("Error: Usage createdatafile.py uh2descnamelistfile luh3descrawfile luh3descrawdir workrawfilename workrawfiledir regionfile luh3descsurffile luh3descsurfdir")
    sys.exit()
else:
...
for timeindex in range(numyears):
    ...
    for cftindex in range(cftnum):
        ...
        if (cftindex < 10):
            PCTCFTfilename = luh3descdirname + luh3descrawinputname + ".PCTCFT0" + str(cftindex) + "." + str(timeyear) + ".dat"
        else:
            PCTCFTfilename = luh3descdirname + luh3descrawinputname + ".PCTCFT" + str(cftindex) + "." + str(timeyear) + ".dat"
    
        print("Processing PCTCFT > " + PCTCFTfilename)

        insurffloatdata = np.fromfile(PCTCFTfilename, dtype="float32", count=-1)
        insurffloatgrid = np.reshape(insurffloatdata,(subregionlatcells,subregionloncells))
        lsmPCTCFT[cftindex,timeindex,:,:] = insurffloatgrid[::-1,:]
